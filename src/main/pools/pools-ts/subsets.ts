...
export function isBytesData(d: BytesData | undefined): d is BytesData {
  return !!d && typeof d.bitLength === 'number' && d.data instanceof Uint8Array
}

export function isPackedData(d: PackedData | undefined): d is PackedData {
  return !!d && typeof d.bitLength === 'number' && Array.isArray(d.data)
}

export function isSubsetData(d: SubsetData | undefined): d is SubsetData {
...
export function subsetDataToBytes(subsetData: SubsetData): BytesData {
  ...
  for (let i = 0; i < (tail === 0 ? 8 : tail); i++) {
    ...
    bytesData[bytesLength - 1] = bytesData[bytesLength - 1] | (v << (7 - i))
  }
...
export function unpackSubsetData(packedData: PackedData): SubsetData {
  const pd = packedData as unknown
  if (!isPackedData(packedData)) {
    throw new Error(`Expected PackedData input only, received ${pd ? pd.toString() : pd} instead`)
  }
...
// Helper function for generating full subsetData from an array of One indexes
export const generateSubsetData = (array: number[]) => {
  const subsetData: SubsetData = []
  for (let i = 0; i < array.length; i++) {
    ...
    // next itters
    if (i > 0) {
      const prevVal = array[i - 1]
      for (let j = prevVal + 1; j <= array[i]; j++) {
        if (j > 0) {
          if (j !== array[i]) {
            subsetData.push(0)
          } else {
