...
*/
type ExtendedSeed = [presecret: BigNumber, chainCode: BigNumber, randomness: BigNumber]
...
*/
type HDNode = {
  extendedSeed: [presecret: Seed[0], chainCode: Seed[1], randomness: BigNumber]
  keyPair: KeyPair
}
...
export class HDSecretGenerator {
  ...
  constructor({ mnemonic, password = '', numKeys = 10 }: { mnemonic: string; password?: string; numKeys?: number }) {
    if (!isValidMnemonic(mnemonic)) {
      throw new Error('Invalid mnemonic')
    }
    const seedHex = mnemonicToSeed(mnemonic, password)
    this._mainSeed = [BigNumber.from(seedHex.slice(0, 66)).mod(P), BigNumber.from('0x' + seedHex.slice(66, 130)).mod(P)]
    this._nodes = []
  ...
  public nullifiersAt(index: number, numNullifiers: number, contractCode: BigNumber): BigNumber[] {
    const nullifiers = []
  ...
  public nullifiersAtRange(start: number, end: number, contractCode: BigNumber): BigNumber[] {
    const nullifiers = []
