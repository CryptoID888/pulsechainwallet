...
import { MerkleTree, type MerkleTreeJSON, type MerkleProof } from './merkle_tree'
import {
  unpackSubsetData,
  bytesToSubsetData,
  isBytesData,
...
export class AccessList {
  ...
  constructor({
    ...
    this.tree = new MerkleTree({ leaves, zero, onProgressUpdate })
  }
  ...
  public setWindow(start: number, end: number, newSubsetData: SubsetData): void {
    if (isNaN(start) || isNaN(end)) {
      throw new Error(`Invalid start or end, expected number`)
    } else if (start < 0 || end < 0 || start > end || end > this.tree.CAPACITY || end > this.tree.length) {
  ...
  static fullEmpty({ subsetLength, accessType }: { subsetLength: number; accessType: string }): AccessList {
    const accessList = new AccessList({ accessType })
