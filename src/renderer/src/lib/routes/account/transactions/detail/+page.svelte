<script lang="ts">
  ...
  $: chainId = transaction.chainId || chainIdToChain.get(943)!.id
  $: chain = chainIdToChain.get(chainId)!
  $: visualChain = chainById.get(chainId)!
  $: {
    state.transactionData(chainId, transaction.hash).then((data) => {
      chaindata = data
    })
  }
  $: href = `${chain.blockExplorers.default.url}${chainId === pulsechainV4.id ? '/#' : ''}/tx/${chaindata?.transaction?.hash}`
  $: status = chaindata?.receipt?.status || 'pending'
</script>

<Crumb {...crumbs.transactions} />
<Crumb {...crumbs.transactionDetails(transaction.hash)} />
