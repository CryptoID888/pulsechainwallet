import type { Block, BlockTag, GetEnsAddressReturnType, Hex, SendTransactionParameters, Transaction, TransactionReceipt } from "viem"
import type { ChainIds, Config } from "./config"
import type { Contact, TransactionData } from "./types"
import type { ChainTransaction } from "./types"
import type { Erc20Token } from "./token"
import type { QueryKey } from "./indexer"
import type { Deposit, PrivacyPool, Query } from "./indexer/gql/graphql"
import type { UpdateableWalletMetadata } from "./wallets"
import type { WalletMetadata, Account } from "./wallets"
import type { NonceData } from "./wallets"
import type { PathTypes } from "./path"
import type { Proof } from "./pools"
...
export interface API {
  ...
  'config:get': () => Config,
  'config:set': (key: string | string[], value: any) => void,

  'sql:run': (sql: string, params: any[]) => RunResult,
  'sql:get': (sql: string, params: any[]) => any,
  'sql:all': (sql: string, params: any[]) => any[],
  ...
  'wallet:sendTransaction': (chainId: ChainIds, account: Account, input: SendTransactionParameters, action: string) => Hex,
  'wallet:queueTransaction': (chainId: ChainIds, account: Account, input: Omit<SendTransactionParameters, 'maxFeePerGas' | 'maxPriorityFeePerGas' | 'nonce'>, action: string) => Hex,
...
export type InvokerDefaultNull<T extends keyof API> = (...args: Parameters<API[T]>) => Promise<ReturnType<API[T]> | null>

export type InvokerRequireType<T extends keyof API, R = any> = <I extends R>(...args: Parameters<API[T]>) => Promise<I>

export type InvokerRequireTypeDefaultNull<T extends keyof API, R = any> = <I extends R | null>(...args: Parameters<API[T]>) => Promise<I | null>

export type HandlerFn<T extends keyof API> = (event: IpcMainInvokeEvent, ...params: Parameters<API[T]>) => Promise<ReturnType<API[T]>>

export type DirectHandler<T extends keyof API> = (...params: Parameters<API[T]>) => Promise<ReturnType<API[T]>> | ReturnType<API[T]>

export const handler = <T extends keyof API>(fn: DirectHandler<T>) => (
  (_event: IpcMainInvokeEvent, ...params: Parameters<typeof fn>) => fn(...params)
)
